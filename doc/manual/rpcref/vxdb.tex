...
\rpcparam{path}{Absolute path \emph{inside} the virtual server (this value may
	be empty to retrieve information about all configured disk limits)}
\end{rpcsynopsis}
...
\rpcreturnparam{path}{Absolute disk limit path \emph{inside} the virtual
	server}
...
\rpcparam{name}{Unique virtual server name}
\rpcparam{path}{Absolute disk limit path \emph{inside} the virtual server (this
value may be empty to remove all configured limits)}
\end{rpcsynopsis}
...
\begin{rpcsynopsis}{vxdb.dx.limit.set}{string name, string path, uint32 space,
	uint32 inodes, int reserved}
\rpcparam{name}{Unique virtual server name}
\rpcparam{path}{Absolute disk limit path \emph{inside} the virtual server}
...
\begin{rpcreturncomplex}{\texttt{struct}}{string init, string halt,
	string reboot}
...
\rpcreturnparam{dst}{Absolute destination path for the mount point inside the
	virtual server (this value may be empty to return a list of all configured
	mount points)}
...
\rpcparam{xid}{Context ID to lookup}
\end{rpcsynopsis}
...
\rpcreturnsimple{an \texttt{array} of \texttt{string}s - one for each owner - }

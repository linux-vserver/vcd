...
void usage(int rc)
{
	printf("Usage: vce <opts>* <command>\n"
	       ...
	       "\n"
	       "   init.method.get     <name>\n"
	       "   init.method.set     <name> <method> [<timeout> [<start> [<stop>]]]\n"
	       "\n"
	       "   mount.get           <name> [<path>]\n"
	       "   mount.remove        <name> [<path>]\n"
	       "   mount.set           <name> <path> <spec> <vfstype> <mntops>\n"
	       "\n"
	       "   nx.addr.get         <name> [<addr>]\n"
	       "   nx.addr.remove      <name> [<addr>]\n"
	       "   nx.addr.set         <name> <addr> [<netmask> [<broadcast>]]\n"
	       "\n"
	       "   nx.broadcast.get    <name> [<addr>]\n"
	       "   nx.broadcast.remove <name> [<addr>]\n"
	       "   nx.broadcast.set    <name> <addr> [<netmask> [<broadcast>]]\n"
	       "\n"
	       ...
	       "   vx.sched.set        <name> <bucket> [<cpuid>]\n"
	       "\n"
	       "   <bucket> = <int> <fill> <int2> <fill2> <min> <max> <bias>\n"
	       "\n"

...
static const char rcsid[] = "#(@) $Id: xml_element.c,v 1.11 2005/03/17 22:41:15 danda Exp $";
 ...
 * HISTORY
 *   $Log: xml_element.c,v $
 *   Revision 1.11  2005/03/17 22:41:15  danda
 ...
 *
 *   Revision 1.10  2002/07/05 00:24:20  danda
...
{
   return fptr && text ? fptr(data, text, len ? len : (int) strlen(text)) : 0;
}
...
#define should_escape(c, flag) ( ((flag & xml_elem_markup_escaping) && markup(c)) || \
  ...
  if(buf && *buf) {
    ...
    iLength = old_len ? old_len : (int) strlen(buf);
    while(*bufcopy) {

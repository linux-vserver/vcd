...
list_t *sched_list_init(void)
{
	...
	LIST_ADD_SCHED(VXSM, FILL_RATE2)
	LIST_ADD_SCHED(VXSM, INTERVAL2)
	LIST_ADD_SCHED(VXSM, TOKENS)
	LIST_ADD_SCHED(VXSM, TOKENS_MIN)
	LIST_ADD_SCHED(VXSM, TOKENS_MAX)
	LIST_ADD_SCHED(VXSM, PRIO_BIAS)

	LIST_ADD_SCHED(VXSM, IDLE_TIME)
	LIST_ADD_SCHED(VXSM, FORCE)
	LIST_ADD_SCHED(VXSM, CPU_ID)
	LIST_ADD_SCHED(VXSM, BUCKET_ID)
